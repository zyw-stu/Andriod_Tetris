...
data class TetrisState(
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (!brickArray.contentDeepEquals(other.brickArray))
            return false
        if (tetris != other.tetris)
            return false
        if (gameStatus != other.gameStatus)
            return false
        if (soundEnable != other.soundEnable)
            return false
        if (nextTetris != other.nextTetris)
            return false
        return true
    }
    ...
    if (canDown()) {
        ...
        return if (gameOver) {
            ...
            if (clearRes == null) {
                ...
                // infoStorage.currentScore = currentScore + 10 加在这里是每次消除+10分, 不对
                copy(
...
private fun TetrisState.clearIfNeed(): TetrisState? {
    ...
    while (true) {
        ...
        if (lastLine.find { it == 0 } == null) {
            ...
            infoStorage.currentScore = currentScore + 10
            currentScore += 10

            if(infoStorage.highestScore < currentScore) {
                infoStorage.highestScore = currentScore
            }
        }
        else {
            index--
